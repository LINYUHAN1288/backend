...
type User struct {
	Id         int    `orm:"column(userid);auto" description:"涓婚敭ID"`
	Username   string `orm:"column(username);size(32)"`
	Userpass   string `orm:"column(userpass);size(32)"`
	Useremail  string `orm:"column(useremail);size(100)"`
	Createtime uint   `orm:"column(createtime)"`
}
...
func ValidatePass(m *User) (err error) {
	o := orm.NewOrm()
	v := &User{Username: m.Username}
	if err = o.Read(v, "username"); err == nil {
		if v.Userpass == m.Userpass {
			return nil
		} else {
